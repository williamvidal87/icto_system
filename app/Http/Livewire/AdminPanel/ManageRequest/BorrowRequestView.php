...
class BorrowRequestView extends Component
{
    use WithFileUploads;
...
// end sample
        ...
        try {
            if($this->BorrowEquipmentRequestID){
                ...
                $log_data = ([
                    ...
                    'activity'      =>  'This Request No. '.'BR'.(2200+$this->BorrowEquipmentRequestID).' is successfully Approve to Borrow Equipment Request',
                    'created_at'    =>  date('Y-m-d H:i:s')
                ]);
                UserActivityLogsDatabase::create($log_data);
                
                $this->data = ([
                    'borrow_id'                 =>  $this->BorrowEquipmentRequestID,
                    'request_no'                =>  'BR'.(2200+$this->BorrowEquipmentRequestID),
                    'client_id'                 =>  $this->client_id,
                    'user_office_info'          =>  $this->user_office_info,
                    'status_id'                 =>  2,
                    'personnel_id'              =>  $this->personnel_id,
                    'request_category'          =>  3,//borrow type
    ...
    {
        return view('livewire.admin-panel.manage-request.borrow-request-view',[
